...
2. **Custom TCP - TCP Range from 30000 to 32768:** Reserved for Kubernetes NodePort Service, enabling communication within the specified port range.
...
sudo docker ps
```
After the docker installation, we create a Nexus container (Remember added 8081 port in the security group)
...
```
git branch: 'main', url: 'https://github.com/naveensilver/Ekart-Project.git'
```
...
```
Now the Jenkins Pipeline after Stage 7 
```
pipeline {
    agent any
    tools{
...
sh "docker build -t naveensilver/ekart:latest -f docker/Dockerfile ."
```
...
sh "trivy image naveensilver/ekart:latest"
```
...
```bash
sh "trivy image naveensilver/ekart:latest > trivy_report.txt"
```
...
stage('PUSH Image to DTR') {
    steps {
        script {
            withDockerRegistry(credentialsId: 'dockerhub-credentials') {
               sh "docker push naveensilver/ekart:latest"
            }
        }
    }
}
...
```yaml
- Change the Image name to "image: naveensilver/ekart:latest"
